...
if(!class_exists('WP_Widget_Related_Upsell_Product')){
	class WP_Widget_Related_Upsell_Product extends WP_Widget {
		...
		function WP_Widget_Related_Upsell_Product() {
			...
			$this->WP_Widget('wd_related_upsell_product', __('WD - Related Upsell Product','wpdance'), $widgetOps);
		}
		...
		function widget( $args, $instance ) {
			...
			?>
			<?php if($related_upsell_products->post_count>0){
			$i = 0; 
			$random_id = 'wd_widget_related_upsell_wrapper_'.rand();
			?>
			<div class="wd_widget_related_upsell_wrapper woocommerce loading <?php echo ($show_nav)?'has_navi':''; ?>" id="<?php echo $random_id; ?>" >
