			...
			if ( $products->have_posts() ) : ?>
				...
				<div class="featured_product_wrapper <?php echo 'style-'.$style; ?> <?php echo ((int)$show_rating)?'has_rating':''; ?>" id="<?php echo $_random_id;?>">
					<div class="featured_product_wrapper_meta"> 
						<?php
							if(strlen(trim($title)) >0)
								echo "<div class='wp_title_shortcode_products'><h3 class='heading-title title'>{$title}</h3></div>";
							if(strlen(trim($desc)) >0)	
								echo "<p class='desc-wrapper'>{$desc}</p>";
						?>
					</div>
		...
		function wd_custom_categories_function($atts,$content){
			...
			<?php $_random_id = 'featured_categories_slider_wrapper_'.rand(); ?>
				...
				//<![CDATA[
					...
					if(!_show_item_title){
						jQuery(_random_id + " ul.products li.product h3").remove();
					}
		...
		function wd_product_filter_by_sub_category_function($atts,$content){
			...
			?>
				...
				<div class="featured_product_wrapper <?php echo 'style-'.$style; ?> <?php echo ((int)$show_rating)?'has_rating':''; ?>" id="<?php echo $_random_id;?>">
					<div class="featured_product_wrapper_meta"> 
						<?php
							echo "<div class='wp_title_shortcode_products'><h3 class='heading-title'>{$title}</h3>";
							?>
					...
					<?php 
						foreach($_sub_prod_cat as $sub_cat){
							$args = array(
								'post_type'	=> 'product',
								'post_status' => 'publish',
								'ignore_sticky_posts'	=> 1,
								...
								'tax_query' => array(
									array(
										'taxonomy' => 'product_cat',
										'terms' => array( esc_attr($sub_cat->slug) ),
										'field' => 'slug',
										'include_children' => false
									)
								)
							...
							if ( $products->have_posts() ){
								?>
								<div style="display: none" data-slug="<?php echo $sub_cat->slug; ?>" class="featured_product_wrapper_inner  <?php echo (!(int)$show_label_title)?'wd_hide_label_title':''; ?>">
				...
				<script type="text/javascript">
					jQuery(document).ready(function(){
						...
						_random_id.find('.wd_list_categories ul li a.link_cat').each(function(index,element){
							...
							if( _random_id.find('.featured_product_wrapper_inner[data-slug='+_data_slug+']').length == 0 )
						...
						if( typeof _data_slug != "undefined" ){
							...
							_random_id.find('.featured_product_wrapper_inner[data-slug='+_data_slug+']').show().addClass('current');
						}
						...
						_random_id.find('.wd_list_categories ul li a.link_cat').live('click', function(){
							...
							var _data_slug = jQuery(this).attr('data-slug');
							_random_id.find('.wd_list_categories ul li a.link_cat').each(function(index,ele){
								var data_slug = jQuery(ele).attr('data-slug');
								if( _data_slug == data_slug )
									jQuery(ele).addClass('current');	
							});
							
							_random_id.find('.featured_product_wrapper_inner.current').fadeOut(
									300, function(){
										_random_id.find('.featured_product_wrapper_inner[data-slug='+_data_slug+']').fadeIn(250).addClass('current');
									}
								).removeClass('current');
	...
	if(!function_exists('wd_custom_products_category_function')){
		function wd_custom_products_category_function($atts,$content){
			...
			<div class="wd_custom_category_shortcode <?php echo (!(int)$show_label_title)?'wd_hide_label_title':''; ?> <?php echo ((int)$show_rating)?'has_rating':''; ?>" id="<?php echo $_random_id; ?>">
				<div class="wp_title_shortcode_products">
			...
			<script type="text/javascript">
				...
				jQuery(document).ready(function(){
					...
					//Change product
					<?php if( !$_is_mobile ): ?>
					_random_id.find('.wd-custom-category-right-wrapper ul li').wd_custom_product_shortcode_change_product();
					<?php endif; ?>
					  
				});
				jQuery.fn.wd_custom_product_shortcode_change_product = function(){
					jQuery(this).find('.product_thumbnail_wrapper a').bind('click',function(event){
						...
						li_left.wd_custom_product_shortcode_change_product();
						//append product
